...
class PrivateObjectFactory {
  ...
  initializeRec(recObject) {
    return new Rec(recObject.id, recObject.title || "", recObject.description || "", recObject.location || "", recObject.duration || 0, recObject.startTime || "", recObject.tags || [], // need to change, db has tagId and points to tag table
    recObject.draft || true, recObject.contactInfo || "", recObject.websiteLink || "", recObject.rules || "", recObject.newFields || {});
  }
  ...
  initializeUser(userObject) {
    return new User(userObject.id, userObject.email, userObject.fullName || "", userObject.passwordHash, userObject.preferredEvents || [] // may need to parse list
    );
...
module.exports = ObjectFactory; // I think this is the best way to create a singleton?
